...
class User:
    ...
    def __str__(self):
        ...
        if self.owned_flags:
            ...
            result += "star (total {}) - {:0.2f}.\n".format(total_stars, user_value / total_stars)  # exception if total_stars is equal to 0
...
available_flags = [
    ...
    Flag("Russia",     1200, "Home to the Hermitage Museum", 3, 3, 0),  # stars = 0 --- can cause divide by zero exception
    ...
    for _ in range(amount):
        ...
        try:
            if 0 <= flag_index < len(available_flags):
                ...
                allowed_to_buy_flag = user.can_afford_flag(flag_to_buy)  # if we get an exception, it would be possible to buy the CSA flag without the "Additional Check"
                log_message = str(user) + f" is trying to buy flag {flag_to_buy.name}" # calling to the __str__ method of class User - it can cause an exception
...
def sell_flag_menu():
    ...
    flag_to_sell = None  # BUG! not setting it to None in each iteration
    for _ in range(amount):
...
def main_menu():
    ...
    send("Hope you had Fun with Flags With Flags! Goodbye.")
